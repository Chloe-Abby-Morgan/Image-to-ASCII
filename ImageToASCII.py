...
output_file_path = r"C:\Users\chloe\Desktop\Dev\Image to ASCII" + r"\output.txt"
image_width = 100
image_height = 100
...
for row_index in range(len(img_array)):
    for pixel_index in range(len(img_array[row_index])):
        pixel_char = char_list[round(((len(char_list)-1)/256) * img_array[pixel_index, row_index])]
